...
void RenderSystem::handleNewGame(const Message&)
{
	over_ = false;
}
...
void RenderSystem::drawFighters() {
	...
	auto fighter = mngr_->getSystem<NetworkSystem>()->getFighter();
	drawBox(mngr_->getEntities(ecs::_grp_FIGHTERS)[fighter]);
}
