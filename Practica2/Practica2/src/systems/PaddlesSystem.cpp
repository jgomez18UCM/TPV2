...
void PaddlesSystem::initSystem() {
	auto ballEntity = mngr_->getHandler(ecs::_hdlr_BALL);
	ballTr_ = mngr_->getComponent<Transform>(ballEntity);
	assert(ballTr_ != nullptr);

	// create the left paddle
	auto leftPaddle = mngr_->addEntity(ecs::_grp_PADDLES);
...
//	leftPaddleCtrl->setType(PaddleCtrl::MOUSE);
	...
	auto rightPaddle = mngr_->addEntity(ecs::_grp_PADDLES);
...
void PaddlesSystem::movePaddle(ecs::Entity *paddle) {
